...
import FragShader from "./glsl/glareWaterNoiseFragment.js"
...
const inputAlpha = document.querySelector("#use-alpha")
let iTime = 0;
let colorRev = false;
let colorRem = false;
...
const materialPlane = new THREE.ShaderMaterial({
    ...
    uniforms: {
        ...
        useAlpha:  {value:   inputAlpha.checked}
    }
})
// 监听传参数
params.addEventListener('input', (e) => {
    if(e.target.nodeName === "INPUT"){
        ...
        materialPlane.uniforms.useAlpha.value   = inputAlpha.checked
    }
})
// 开关Noise动画模式
...
btnReNoiseElementSize.addEventListener('click', () => {
    materialPlane.uniforms.eleSize.value = 1.5
    inputNoiseEleSize.value = 1.5
})
btnReNoiseDetail.addEventListener('click', () => {
    materialPlane.uniforms.detail.value = 4
    inputNoiseDetail.value = 4
})
btnReNoiseAlpha.addEventListener('click', () => {
    materialPlane.uniforms.alpha.value = 1
    inputNoiseAlpha.value = 1
})
btnReColor2.addEventListener('click', () => {
    materialPlane.uniforms.color2.value = hexToRgb("#41d8ec")
    inputColorChoose2.value = "#41d8ec"
})
...
btnVideoStop.addEventListener('click', () => {
    ...
    saveAs(videoBlob, 'Glare Water Noise.mp4')
