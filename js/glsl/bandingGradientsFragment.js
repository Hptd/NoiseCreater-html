...
void main(){
    ...
    float uIter = broken; // 重复次数
	float st = fract(uv.y*uIter);
	float off = floor(uv.y*uIter);
	float N = billowedNoise(vec3(off,iTime*onlyBri,floor(uv.y))); // 0.2 节奏速度（0-1）
	float pos = fract(uv.x - iTime*delicate + N ) *st; // 0.8 位移速度 （0-2）

    vec3 color = vec3(1.0 - pos);
