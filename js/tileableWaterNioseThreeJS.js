...
import FragShader from "./glsl/tileableWaterNoiseFragment.js"
...
btnReContrast.addEventListener('click', () => {
    materialPlane.uniforms.contrast.value = 0.005
    inputContrast.value = 0.005
})
btnReSubdivide.addEventListener('click', () => {
    materialPlane.uniforms.subdivide.value = 4
    inputSubdivide.value = 4
})
btnReColor1.addEventListener('click', () => {
    materialPlane.uniforms.color1.value = hexToRgb("#005980")
    inputColorChoose1.value = "#005980"
})
...
btnVideoStop.addEventListener('click', () => {
    ...
    saveAs(videoBlob, 'Tileable Water Noise.mp4')
